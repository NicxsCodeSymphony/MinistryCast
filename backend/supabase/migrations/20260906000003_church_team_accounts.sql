-- Church admins can list/update accounts in their own church (Settings → Team).
...
REVOKE ALL ON FUNCTION public.church_list_accounts() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.church_list_accounts() TO authenticated;
...
REVOKE ALL ON FUNCTION public.church_update_account(uuid, text, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.church_update_account(uuid, text, text, text) TO authenticated;
