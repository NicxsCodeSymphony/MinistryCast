...
export default function VerseOverlay({
  ...
  const styleObj: LyricTextStyle = useMemo(() => {
    if (!effectiveTextStyle?.trim()) {
      return { bold: true, italic: false, underline: false };
    }
    // Prefer comma flags (bold,italic) used elsewhere; fall back to JSON.
    if (
      effectiveTextStyle.includes("{") ||
      effectiveTextStyle.trim().startsWith("{")
    ) {
      try {
        const parsed = JSON.parse(effectiveTextStyle) as Partial<LyricTextStyle>;
        return {
    ...
    return parseLyricTextStyle(effectiveTextStyle);
  }, [effectiveTextStyle]);

  const fontWeight = styleObj.bold ? 700 : 400;
            ...
            >
              <div className="text-center mb-[0.9em]">
                <p
                  ...
                  style={{ fontSize: "0.62em", fontWeight }}
                >
                  {language}
                </p>
                <div className="flex items-center justify-center gap-4 mt-[0.45em]">
                  ...
                  <h2
                    className={styleObj.underline ? "underline underline-offset-[0.18em]" : undefined}
                    style={{
                      fontSize: "1em",
                      lineHeight: 1.15,
                      fontWeight,
              ...
              <div
                ...
                style={{ color: foreground, fontWeight }}
              >
                {visible.map((row) => (
                  <p
                    key={row.verse}
                  ...
                  >
                    <span
                      className="mr-[0.4em] tabular-nums"
                      style={{
                        fontWeight,
                        textDecoration: styleObj.underline
                          ? "underline"
                          : undefined,
