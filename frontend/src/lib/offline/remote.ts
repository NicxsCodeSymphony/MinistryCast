...
async function writeRows(
  ...
  if (!error) {
    // Upsert RETURNING can be empty when SELECT RLS hides rows even though write succeeded.
    if (!data?.length && mode === "insert") {
...
function requiredParentRef(table: TableName, column: string, row: LocalRow) {
  if (table === "setlist_items") return requiredSetlistRef(row, column);
  if (table === "sermon_slides" && column === "sermon_id") return true;
  if (table === "sermon_notes" && column === "sermon_id") return true;
  if (table === "song_lyric_sections" && column === "song_id") return true;
  if (table === "setlist_shares" && column === "setlist_id") return true;
  if (table === "sermon_shares" && column === "sermon_id") return true;
  return false;
}
...
async function prepareRow(
  ...
  for (const ref of refs) {
    ...
    const required = requiredParentRef(table, ref.column, row);
    if (!relatedId || typeof relatedId !== "string") {
      if (required) {
        throw new SkipOutboxItem(`Missing ${ref.column} for ${table}.`);
      }
      continue;
    }
    try {
...
async function applyItem(item: OutboxItem) {
  ...
  // replaceChildren: parent must exist before wipe+insert (RLS checks sermon_id).
  if (op.parentColumn === "sermon_id") {
    const ok = await pushLocalRow(item.churchId, "sermons", op.parentId, stack);
    if (!ok) {
      throw new SkipOutboxItem(`Missing sermon for ${op.table}.`);
    }
  } else if (op.parentColumn === "song_id") {
    const ok = await pushLocalRow(item.churchId, "songs", op.parentId, stack);
    if (!ok) {
      throw new SkipOutboxItem(`Missing song for ${op.table}.`);
    }
  } else if (op.parentColumn === "setlist_id") {
    const ok = await pushLocalRow(item.churchId, "setlists", op.parentId, stack);
    if (!ok) {
      throw new SkipOutboxItem(`Missing setlist for ${op.table}.`);
    }
  }
  ...
  // Upsert so a partial delete / retry does not fail on duplicates.
  await writeRows(op.table, prepared, "upsert");
  for (const row of op.rows) {
