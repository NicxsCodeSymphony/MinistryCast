...
import { cachedQuery, invalidateQueryPrefix } from "./offline/queryCache";
import {
  getRow,
  listRows,
  putRow,
...
export { readSetlistViewChurchId, writeSetlistViewChurchId };
...
export async function listSermons(
  ...
  return cachedQuery(
    `listSermons:${churchId}:${options.query ?? ""}:${offset}:${limit}:${options.viewChurchId ?? ""}:${archived ? "1" : "0"}`,
    async () => {
  ...
  for (const row of sermons) {
    if (row.archived_at || !row.service_date) continue;
    const dayEnd = new Date(`${row.service_date}T23:59:59`);
    if (!Number.isFinite(dayEnd.getTime()) || dayEnd.getTime() >= nowMs) continue;
    const stamped = stamp();
    const next = {
      ...row,
      status: "archived",
    ...
    await putRow(churchId, "sermons", next as LocalRow);
    row.archived_at = stamped;
...
export async function listSetlists(
  ...
  const archived = Boolean(options.archived);
  return cachedQuery(
    `listSetlists:${churchId}:${options.query ?? ""}:${offset}:${limit}:${options.viewChurchId ?? ""}:${archived ? "1" : "0"}`,
    async () => {
  ...
  // Past service dates auto-archive into the Archive tab (after that calendar day).
  const nowMs = Date.now();
  for (const row of rows) {
    if (row.archived_at || !row.service_at) continue;
    const serviceDayEnd = new Date(row.service_at);
    serviceDayEnd.setHours(23, 59, 59, 999);
    if (
      !Number.isFinite(serviceDayEnd.getTime()) ||
      serviceDayEnd.getTime() >= nowMs
    ) {
      continue;
    }
    const stamped = stamp();
    ...
    await putRow(churchId, "setlists", next as LocalRow);
    row.archived_at = stamped;
    row.updated_at = stamped;
  }
...
export async function deleteSetlist(id: string) {
  ...
  // Drop stale list/detail caches so reload doesn't reopen a deleted id.
  invalidateQueryPrefix(`listSetlists:${churchId}:`);
  invalidateQueryPrefix(`getSetlist:${churchId}:${id}`);
}
...
export async function archiveSetlistAfterSession(setlistId: string) {
  const churchId = await ready();
  const setlist = await getRow<Setlist>(churchId, "setlists", setlistId);
  if (!setlist) return;
  const now = stamp();
  if (!setlist.archived_at) {
    await putRow(churchId, "setlists", {
      ...setlist,
      archived_at: now,
      updated_at: now,
    } as LocalRow);
  ...
  const items = (await listRows<SetlistItem>(churchId, "setlist_items")).filter(
    (row) => row.setlist_id === setlistId && row.sermon_id,
  );
  for (const item of items) {
    if (!item.sermon_id) continue;
    const sermon = await getRow<Sermon>(churchId, "sermons", item.sermon_id);
    if (!sermon || sermon.archived_at) continue;
    await putRow(churchId, "sermons", {
      ...sermon,
      status: "archived",
      archived_at: now,
  ...
  invalidateQueryPrefix(`listSetlists:${churchId}:`);
  invalidateQueryPrefix(`listSermons:${churchId}:`);
  invalidateQueryPrefix(`getSetlist:${churchId}:${setlistId}`);
}

export async function setSetlistArchived(id: string, archived: boolean) {
  const churchId = await ready();
  const existing = await getRow<Setlist>(churchId, "setlists", id);
  if (!existing) throw asError(null, "Could not update setlist.");
  const now = stamp();
...
export async function setSermonArchived(id: string, archived: boolean) {
  ...
  await putRow(churchId, "sermons", {
    ...existing,
    status: archived ? "archived" : existing.status === "archived" ? "ready" : existing.status,
    archived_at: archived ? now : null,
    updated_at: now,
  } as LocalRow);
  invalidateQueryPrefix(`listSermons:${churchId}:`);
  invalidateQueryPrefix(`getSermon:${churchId}:${id}`);
}
...
export async function startPresentation(setlistId: string, name?: string) {
  ...
  // Going live again should revive a setlist that was archived on the last End.
  await setSetlistArchived(setlistId, false).catch(() => undefined);
  const setlist = await getSetlist(setlistId, { fresh: true });
  ...
  for (const row of live) {
    ...
    // Don't archive the setlist we're about to go live with again.
    if (row.setlist_id && row.setlist_id !== setlistId) {
      await archiveSetlistAfterSession(row.setlist_id);
    }
  }
  ...
  const row: Presentation = {
    ...
    verse_overlay_font: settings?.default_font ?? null,
    verse_overlay_text_size: settings?.lyrics_text_size ?? "48",
    verse_overlay_text_style: "bold",
    verse_overlay_color: "black",
    created_at: now,
    updated_at: now,
  };
...
export async function updatePresentation(
  ...
  patch: Partial<{
    ...
    verse_overlay_font: string | null;
    verse_overlay_text_size: string | null;
...
export async function endPresentation(id: string) {
  const churchId = await ready();
  const existing = await getRow<Presentation>(churchId, "presentations", id);
  const row = await updatePresentation(id, {
    status: "ended",
    ended_at: stamp(),
  });
  if (existing?.setlist_id) {
    await archiveSetlistAfterSession(existing.setlist_id);
  }
  return row;
}
...
export function buildCues(setlist: Setlist): LiveCue[] {
  ...
  for (const item of setlist.items ?? []) {
    if (item.item_type === "song") {
      ...
      const sections = coerceLyricSections(
        [...(item.song?.lyric_sections ?? [])].sort(
      ...
      sections.forEach((section, index) => {
        ...
        const lyricId =
          (item.song?.lyric_sections ?? []).find(
            (row) =>
              row.section === section.section && row.content === section.content,
