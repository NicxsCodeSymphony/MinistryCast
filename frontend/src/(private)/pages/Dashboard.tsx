...
import { getSessionProfile, isSuperadmin, canManageSetlists, isLibraryEditor } from "../../lib/auth";
...
export default function Dashboard() {
  ...
  const load = async () => {
    ...
    setCanEditLibrary(isLibraryEditor(profile));
    ...
    const pick =
      (preferred && listed.items.find((row) => row.id === preferred)) ||
      listed.items[0] ||
      null;
    if (pick) setSetlist(await getSetlist(pick.id));
        ...
        <div className="col-span-12 lg:col-span-8 glass-card rounded-xl p-6 sm:p-8 flex flex-col justify-between min-h-[400px]">
          <div className="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-start">
            <div>
              ...
              <h3 className="text-[clamp(1.75rem,4vw,3rem)] font-bold tracking-[-0.02em] text-on-surface mb-3 leading-tight">
          ...
          <div className="mt-8 flex gap-4">
            <Link
              ...
              onClick={() => {
                if (setlist) writePreferredSetlistId(setlist.id);
              }}
            ...
            <Link
              to={setlist ? `/setlists?setlist=${setlist.id}` : "/setlists"}
              className="px-6 border border-white/10 rounded-xl hover:bg-white/5 transition-all flex items-center justify-center"
              aria-label="Choose setlist"
              title={t("setlists.choosePresentation")}
            >
          ...
          <div className="glass-card rounded-xl p-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                ...
                <div className="text-left">
                  <span className="text-xs font-semibold block">{t("dash.liveSession")}</span>
                  <span className="text-[10px] text-on-surface-variant">
                    {t("dash.buildHint")}
                  </span>
                </div>
