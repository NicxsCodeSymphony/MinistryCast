...
import { readPreferredSetlistId } from "../../lib/preferredSetlist";
...
export default function Live() {
  ...
  const loadBundle = useCallback(async () => {
    ...
    // Prefer the setlist from Setlists → Go Live / preferred presentation setlist.
    const preferred = readPreferredSetlistId();
    const nextSetlistId =
      requestedSetlist ||
      live?.setlist_id ||
      (preferred && listed.items.some((row) => row.id === preferred)
        ? preferred
        : "") ||
      listed.items[0]?.id ||
      "";
    const liveForSetlist =
      live && (!requestedSetlist || live.setlist_id === requestedSetlist)
        ? live
        : null;
    ...
    if (nextSetlistId) {
      try {
        const next = await getSetlist(nextSetlistId, { fresh: true });
        setlistFp.current = setlistFingerprint(next);
        setSetlist(next);
        setSetlists((prev) =>
          prev.some((row) => row.id === next.id) ? prev : [next, ...prev],
        );
      } catch (err) {
        setSetlist(null);
        setError(
          err instanceof Error ? err.message : "Could not load setlist.",
        );
      }
    } else {
      setSetlist(null);
    }
    setPresentation(liveForSetlist);
    if (liveForSetlist) {
      setParams({ presentation: liveForSetlist.id }, { replace: true });
    } else if (requestedSetlist) {
      setParams({ setlist: requestedSetlist }, { replace: true });
    }
    try {
  ...
  const openVerse = (_raw: string, parsed: ParsedBibleRef) => {
    ...
    patchPresentation({
      ...
      verse_overlay_take: 5,
      verse_overlay_font:
        presentation.verse_overlay_font || font || DEFAULT_STAGE_FONT,
      verse_overlay_text_size:
        presentation.verse_overlay_text_size || lyricSize || "48",
      verse_overlay_text_style:
        presentation.verse_overlay_text_style ||
        serializeLyricTextStyle({ bold: true, italic: false, underline: false }),
      verse_overlay_color: presentation.verse_overlay_color || "black",
    });
  };
  ...
  const goLive = async () => {
    ...
    try {
      ...
      // Refresh local setlist after unarchive / cue rebuild.
      try {
        const refreshed = await getSetlist(setlistId, { fresh: true });
        setlistFp.current = setlistFingerprint(refreshed);
      ...
      const next = cue
        ...
        : row;
      setPresentation(next);
      setParams({ presentation: next.id, setlist: setlistId }, { replace: true });
  ...
  const stopLive = async () => {
    ...
    try {
      ...
      const keepSetlist = presentation.setlist_id || setlistId;
      setParams(
        keepSetlist ? { setlist: keepSetlist } : {},
        { replace: true },
      );
      if (keepSetlist) setSetlistId(keepSetlist);
          ...
          <div className="flex items-center gap-3 sm:gap-4 min-w-0">
            <div className="flex items-center gap-2 px-3 py-1 bg-primary/10 rounded-full border border-primary/20">
              ...
              <span className="text-[12px] font-semibold tracking-[0.05em] uppercase text-primary">
                {live ? t("live.onAir") : t("live.standby")}
              </span>
            </div>
            {live && editMode && (
              ...
              <button
                ...
                onClick={() => {
                  void applyChanges().then(() => {
                    toast.success(t("live.editsSaved"));
                  });
                }}
                className="flex items-center gap-2 px-3 sm:px-4 py-2 rounded-lg bg-primary text-on-primary hover:brightness-110 transition-all active:scale-95 font-semibold disabled:opacity-40"
              >
                <span className="material-symbols-outlined text-[18px]">save</span>
                <span className="text-xs font-semibold sm:inline">
                  {hasPending ? t("live.saveEdits") : t("live.editsSaved")}
                </span>
              </button>
              <button
                type="button"
                ...
                onClick={() => {
                  ...
                  setPendingSermonSizes({});
                  setPendingRosters({});
                  toast.success(t("live.editsDiscarded"));
                }}
                className="flex items-center gap-2 px-3 py-2 rounded-lg border border-white/10 text-on-surface-variant hover:bg-white/5 transition-all disabled:opacity-40"
              >
      ...
      <main className="pt-16 h-screen flex overflow-hidden">
        <aside className="hidden lg:flex w-[280px] border-r border-white/5 bg-surface-container-lowest/50 flex-col h-full overflow-y-auto custom-scrollbar shrink-0">
          <div className="p-4 border-b border-white/5 bg-surface-container-low shrink-0">
            ...
            <div
              ...
              role="tablist"
              aria-label={t("live.setlist")}
            >
              {setlists.length === 0 ? (
                <p className="text-sm text-on-surface-variant">{t("live.noSetlists")}</p>
              ) : (
                setlists.map((row) => {
          ...
          <div className="shrink-0 border-t border-white/5 bg-surface-container-low">
            <button
              type="button"
              onClick={() => navigate("/dashboard")}
              className="w-full flex items-center gap-2 px-4 py-3 text-sm text-on-surface-variant hover:text-on-surface hover:bg-white/5 transition-colors border-b border-white/5"
            >
              <span className="material-symbols-outlined text-[18px]">arrow_back</span>
              <span className="font-medium">{t("live.backToDashboard")}</span>
            </button>
        ...
        <section className="flex-1 flex flex-col bg-surface-container-lowest p-3 sm:p-6 overflow-hidden min-w-0">
          <button
            type="button"
            onClick={() => navigate("/dashboard")}
            className="lg:hidden mb-3 self-start flex items-center gap-1.5 text-sm text-on-surface-variant hover:text-on-surface"
          >
            <span className="material-symbols-outlined text-[18px]">arrow_back</span>
            {t("live.backToDashboard")}
          </button>
          {error ? (
            ...
            <div className="flex-1 rounded-xl overflow-hidden relative border-2 border-primary shadow-[0_0_40px_rgba(155,203,255,0.15)] min-h-[240px]">
              ...
              {displayPresentation?.verse_overlay_ref ? (
                <VerseOverlay
                  ...
                  font={
                    displayPresentation.verse_overlay_font ||
                    font ||
                    DEFAULT_STAGE_FONT
                  }
                  textSize={
                    displayPresentation.verse_overlay_text_size ||
                    lyricSize ||
                    "48"
                  }
                  textStyle={
                    displayPresentation.verse_overlay_text_style || "bold"
                  }
                  color={displayPresentation.verse_overlay_color || "black"}
                  paddingTop={52}
              ...
              <>
                ...
                <select
                  ...
                  onChange={(event) => {
                    ...
                    if (presentation?.verse_overlay_ref) {
                      patchPresentation({ verse_overlay_font: next });
                      return;
                    }
                    setFont(next);
                ...
                <select
                  value={
                    presentation?.verse_overlay_ref
                      ? parseLyricTextStyle(
                          displayPresentation?.verse_overlay_text_style ||
                            "bold",
                        ).bold
                        ? "700"
                        : "400"
                  ...
                  onChange={(event) => {
                    if (!live) return;
                    const bold = event.target.value === "700";
                    if (presentation?.verse_overlay_ref) {
                      const current = parseLyricTextStyle(
                        displayPresentation?.verse_overlay_text_style ||
                          "bold",
                      );
                      patchPresentation({
                        verse_overlay_text_style: serializeLyricTextStyle({
                          ...current,
                          bold,
                        }),
                      });
                ...
                <TextSizePicker
                  ...
                  onChange={(size) => {
                    if (!live) return;
                    if (presentation?.verse_overlay_ref) {
                      patchPresentation({ verse_overlay_text_size: size });
                      return;
                    }
                    setLyricSize(size);
                ...
                <p className="text-[10px] text-on-surface-variant">
                  {presentation?.verse_overlay_ref
                    ? "Font, weight, size, and style update on the projector for this verse overlay."
                      ...
                      <div className="flex items-center justify-between gap-2">
                        ...
                        {editMode ? (
                          <button
                            ...
                            className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-primary text-on-primary text-[11px] font-semibold disabled:opacity-40"
                          >
